...
class MicroscopeApp(pyglet.window.Window):
    ...
    def _finish_ai_move(self, action):
        ...
        fx, fy, tx, ty, jump = action_to_move(action)
        label    = "jump" if jump else "clone"
        ai_label = "MCTS" if self.opponent == "mcts" else f"Minimax-{self.mm_depth}"
        self._apply_move(action)
        if self.state == S_OVER:
            return
        self.state  = S_SELECT
    ...
    def on_draw(self):
        ...
        # Status bar
        shapes.Rectangle(0, 0, WIN_W, INFO_H, color=C_BAR).draw()
        pyglet.text.Label(
            self.status,
...
def main():
    ...
    parser.add_argument("--opponent", choices=["mcts", "minimax"], default="mcts",
    ...
    if args.opponent == "mcts":
        if not args.checkpoint:
            print("Error: --checkpoint is required for MCTS opponent")
            return 1
        ...
        network.eval()
        mcts_agent = MCTS(network, num_simulations=args.simulations,
                          dirichlet_epsilon=0.0)
        print(f"MCTS agent ready  ({args.simulations} sims/move)")
    else:
        print(f"Minimax depth-{args.depth} opponent")

    human_label = args.human_color.capitalize()
    ai_label    = "MCTS" if args.opponent == "mcts" else f"Minimax-{args.depth}"
