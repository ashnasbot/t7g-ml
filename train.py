"""
Training script for CNN architecture.

Usage:
  1. Train from scratch:
     TRAINING_STAGE = "stage1_dense"
...
from lib.networks import MicroscopeCNN, LightweightMicroscopeCNN, DeepMicroscopeCNN

# Training configuration
RESUME_FROM_STAGE = None
LOAD_MODEL_PATH = None

# Progressive curriculum - stages run automatically in sequence
# Train and eval opponents increase gradually for natural learning progression
STAGE_PROGRESSION = [
    # Stage 1: Learn basic mechanics vs random
    # Random opponent is enough to learn: what moves are, how conversion works,
    # that more pieces = higher reward. No planning needed yet.
    {
        "name": "stage1_vs_random",
        "env_class": MicroscopeEnvAggressive,
        "timesteps": 1_500_000,
        "learning_rate": 3e-4,
        "description": "Foundation: learn conversion mechanics vs random",
    ...
    # Stage 3: Train vs depth-5
    {
        ...
        "timesteps": 3_000_000,
        "learning_rate": 1e-4,
        "description": "Mastery: train vs strong minimax-5",
        "train_opponent": "minimax",
        "train_depth": 5,
        "eval_opponent": "minimax",
        "eval_depth": 5
...
def make_env_with_timeout(env_class, render_mode=None, opponent_type=None, minimax_depth=3):
    ...
    The game naturally terminates when no player can move. The timeout is a high
    safety limit to prevent infinite loops from bugs, not a game constraint.
    ...
    for stage_idx in range(start_stage_idx, len(STAGE_PROGRESSION)):
        ...
        if model is None:
            ...
            model = MaskablePPO(
                    ...
                    policy_kwargs={
                        "features_extractor_class": DeepMicroscopeCNN,
